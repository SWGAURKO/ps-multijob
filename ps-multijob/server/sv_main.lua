...
--- Remove jobs that no longer exist in Shared.Jobs and persist cleaned data.
local function CleanInvalidJobs(citizenid, jobs)
    local changed = false
...
QBCore.Functions.CreateCallback("ps-multijob:getJobs", function(source, cb)
    if Config.Core == 'qbox' then 
        ...
        jobs = CleanInvalidJobs(Player.PlayerData.citizenid, jobs)
        local multijobs = {
            whitelist = {},
            civilian = {}
        }
        ...
        for job, grade in pairs(jobs) do
            ...
            if not jobData then
                -- Already removed by CleanInvalidJobs; skip
            else
